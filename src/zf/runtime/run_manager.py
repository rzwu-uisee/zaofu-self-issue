...
RUN_MANAGER_ACTION_EFFECT_PENDING = "run.manager.action.effect.pending"
RUN_MANAGER_ACTION_EFFECT_PASSED = "run.manager.action.effect.passed"
RUN_MANAGER_ACTION_EFFECT_FAILED = "run.manager.action.effect.failed"
...
def run_manager_tick(
    *,
    ...
    """Serialize observe/decide/act/verify into one lease-owned tick."""

    state_dir = Path(state_dir)
    with locked_path(state_dir / "locks" / "run-manager-tick"):
        return _run_manager_tick_unlocked(
            state_dir=state_dir,
            writer=writer,
            config=config,
...
def _run_manager_tick_unlocked(
    *,
    state_dir: Path,
    writer: EventWriter,
    config: ZfConfig | None = None,
    ...
    escalation_terminals = converge_unrecoverable_escalations(
        events,
        writer=writer,
        task_store=TaskStore(state_dir / "kanban.json"),
        request_autoresearch=True,
    )
    if escalation_terminals:
        deterministic_actions_applied += escalation_terminals
        events = _read_events(state_dir, event_log=event_log)
    try:
    ...
    continuation = (
        ...
        if isinstance(projection.get("continuation"), dict)
        else {}
    )
    next_operation = (
        continuation.get("next_operation")
        if isinstance(continuation.get("next_operation"), dict)
        else {}
    )
    selected_operation_key = str(next_operation.get("operation_key") or "")
    if action_filter is not None and not continuation.get("terminal"):
        selected_operation_key = next((
            str(action.get("operation_key") or "")
            for action in projection.get("pending_actions", [])
            if isinstance(action, dict)
            and str(action.get("action") or "") in action_filter
        ), "")
    if auto_execute and selected_operation_key:
        for action in projection.get("pending_actions", []):
            if not isinstance(action, dict):
                continue
            if str(action.get("operation_key") or "") != selected_operation_key:
                continue
            events = _read_events(state_dir, event_log=event_log)
            recovery_admission = recovery_action_admission(events, action)
            if recovery_admission.get("status") != "admitted":
                _emit_blocked_action(
                    writer,
                    {
                        **action,
                        "blocker_kind": "recovery_case_fence",
                        "recovery_admission": recovery_admission,
                    },
                    causation_id=started.id,
                    reason=(
                        "recovery case admission blocked action: "
                        f"{recovery_admission.get('reason') or 'unknown'}"
                    ),
                )
                actions_blocked += 1
                break
            if not _operation_precondition_holds(events, action):
                actions_blocked += 1
                break
            ...
            if decision.get("decision") == "auto_decide":
                ...
                if break_reason:
                    ...
                    _emit_no_progress_run_terminal(
                        ...
                        reason=break_reason,
                        causation_id=(
                            breaker_event.id if breaker_event is not None else started.id
                        ),
                    )
                    actions_blocked += 1
                    break
...
def build_run_manager_projection(
    ...
    *,
    ...
    candidate_action_candidates = _pending_candidate_rework_actions(
        state_dir,
        config,
        events,
        project_root=project_root,
    )
    legacy_replan_sources, legacy_replan_tasks = (
        _legacy_candidate_replan_fallbacks(
            rework_triage_actions,
            candidate_action_candidates,
            config=config,
            events=events,
        )
    )
    if legacy_replan_sources:
        rework_triage_actions = [
            action
            for action in rework_triage_actions
            if not (
                str(action.get("failure_class") or "")
                == "semantic_replan_route_unavailable"
                and str(action.get("source_event_id") or "")
                in legacy_replan_sources
            )
        ]
    ...
    current_run_id = str(goal.get("run_id") or "")
    if current_run_id:
        workflow_actions = [
            action
            for action in workflow_actions
            if _pending_action_matches_run(
                action,
                events=events,
    ...
    attempt_recovery_actions = [
        action
        for action in attempt_recovery_actions
    ...
    candidate_actions = [
        action for action in candidate_action_candidates
        if (
    ...
    workflow_actions = [
        action
        for action in workflow_actions
        if not _workflow_reemit_shadowed_by_candidate(action, candidate_actions)
    ]
    human_gate_actions = []
    ...
    pending_actions, recovery_cases = converge_recovery_actions(
        events,
        pending_actions,
    )
    recovery_metrics_events = events
    if current_run_id:
        from zf.runtime.run_scope import events_for_run

        recovery_metrics_events = events_for_run(events, run_id=current_run_id)
    recovery_metrics = build_recovery_metrics(recovery_metrics_events)
    continuation_identity = build_run_continuation_projection(
        events,
        goal=goal,
        pending_actions=[],
        completion_profile=completion_profile,
    )
    pending_actions = enrich_continuation_actions(
        pending_actions,
        run_id=str(continuation_identity.get("run_id") or ""),
        generation=str(continuation_identity.get("generation") or ""),
        current_progress_digest=str(
            continuation_identity.get("progress_digest") or ""
        ),
    )
    continuation = build_run_continuation_projection(
        events,
        goal=goal,
    ...
    return redact_obj({
        ...
        "summary": {
            ...
            "recovery_effects": dict(recovery_metrics.get("counts") or {}),
        },
    })
...
def _pending_action_priority(action: dict[str, Any]) -> tuple[int, str, str]:
    ...
    if (
        safe_action == "blocked_external_gate"
        and str(action.get("failure_class") or "") == "manual_evidence_required"
    ):
        return (-20, action_name, str(action.get("checkpoint_id") or ""))
    if str(action.get("failure_class") or "") == "no_progress_breaker":
        return (-10, action_name, str(action.get("checkpoint_id") or ""))
    if readiness == "ready_to_execute":
        # A typed contract replan owns recovery for the affected slice.  It
        # must run before an older mechanical batch resume, otherwise that
        # resume can repeatedly trip its no-progress breaker and starve the
...
def build_resident_agent_projection(
    ...
    *,
    ...
    # Resident advice is event-driven.  A healthy run may not need a
    # recommendation for a long time, so silence alone is not a workflow or
    # provider fault.  Pane/process health is owned by run_manager_watchdog and
    # carries an explicit typed failure when recovery is warranted.
    stalled = False
    if not enabled:
        status = "disabled"
    ...
    return redact_obj({
        ...
        "watchdog": {
            ...
            "silent": silent,
            "silence_is_fault": False,
            "reason": (
                "resident agent has not emitted advice; silence alone is not a business fault"
                if silent else ""
            ),
        },
...
def build_run_monitor_projection(
    ...
    *,
    ...
    for task_id, context in _task_attempt_contexts(state_dir / "projections").items():
        if task_id in known_task_ids or int(context.get("open_attempts") or 0) < 1:
            continue
        status = str(context.get("latest_state") or "running")
        assignee = str(context.get("current_owner") or "")
        if assignee:
            lane_counts[assignee] += 1
        in_flight.append({
            "task_id": task_id,
...
def run_goal_completion_claim_event(
    ...
    if cause.type == "goal.closure.synthesized" and closure_result:
        verdict = str(closure_result.get("verdict") or "").strip()
    ...
    if cause.type != "judge.passed" and not result:
        # Historical closure events without an admitted typed result do not
        # carry enough immutable identity to close a run safely.
        return None
    ...
    claim_semantics = {
        ...
        "provider_qualification_status": str(
            result.get("provider_qualification_status") or ""
        ),
...
def run_goal_completion_gate_event(
    ...
    *,
    ...
    blocking_human = [
        decision
        for decision in _blocking_human_decisions(scoped_events)
        if not _admitted_closure_supersedes_legacy_escalation(
            scoped_events,
            claim_payload=claim_payload,
            decision=decision,
        )
    ...
    if candidate_task_ids:
        active_attempts = [
            ...
            if str(item.get("task_id") or "") in candidate_task_ids
        ]
    if active_attempts:
    ...
    if pending_effect_ids:
        blockers.append("pending_action_effect")
    ...
    if (
        canonical_claim
        and bool(claim_payload.get("provider_qualification_required"))
        and str(claim_payload.get("provider_qualification_status") or "")
        != "passed"
    ):
        blockers.append("waiting_external_provider")
    ...
    shared = {
        **({"flow_kind": shared_flow_kind} if shared_flow_kind else {}),
        ...
        "product_acceptance_required": bool(
            claim_payload.get("product_acceptance_required")
        ),
        "product_acceptance_spec_ref": str(
            claim_payload.get("product_acceptance_spec_ref") or ""
        ),
        "product_acceptance_spec_digest": str(
            claim_payload.get("product_acceptance_spec_digest") or ""
        ),
        "product_acceptance_report_ref": str(
            claim_payload.get("product_acceptance_report_ref") or ""
        ),
        "product_acceptance_report_digest": str(
            claim_payload.get("product_acceptance_report_digest") or ""
        ),
        "product_acceptance_verdict": str(
            claim_payload.get("product_acceptance_verdict") or ""
        ),
        "provider_qualification_required": bool(
            claim_payload.get("provider_qualification_required")
        ),
        "provider_qualification_status": str(
            claim_payload.get("provider_qualification_status")
            or "not_required"
        ),
        "goal_coverage": [
            dict(item)
        ...
        return ZfEvent(
            ...
            payload={
                **shared,
                ...
                "active_fanout_count": len(active_fanout_ids),
                "pending_action_effect_count": len(pending_effect_ids),
                "failed_action_effect_count": len(failed_effect_ids),
                "reason": "completion claim has recoverable blockers",
                "external_wait": (
                    {
                        "kind": "provider_qualification",
                        "status": str(
                            shared.get("provider_qualification_status") or ""
                        ),
                        "semantic_attempt_incremented": False,
                    }
                    if "waiting_external_provider" in blockers
                    else {}
                ),
            },
        )
...
def _unresolved_action_effects(events: list[ZfEvent]) -> tuple[list[str], list[str]]:
    ...
    terminal: dict[str, tuple[int, str, dict[str, Any]]] = {}
    for index, event in enumerate(events):
        payload = event.payload if isinstance(event.payload, dict) else {}
        effect_id = str(payload.get("effect_id") or "")
        if not effect_id:
            continue
        if event.type == RUN_MANAGER_ACTION_EFFECT_PENDING:
            pending[effect_id] = (index, payload)
        elif event.type in {
            RUN_MANAGER_ACTION_EFFECT_PASSED,
            RUN_MANAGER_ACTION_EFFECT_FAILED,
        }:
            terminal[effect_id] = (index, event.type, payload)
    pending_ids = sorted(set(pending) - set(terminal))
    failed_ids: list[str] = []
    for effect_id, (index, event_type, _payload) in terminal.items():
        if event_type != RUN_MANAGER_ACTION_EFFECT_FAILED:
            continue
        recovered = any(
        ...
        if not recovered:
            failed_ids.append(effect_id)
    return pending_ids, sorted(failed_ids)


def _candidate_completed_task_ids_for_claim(
    events: list[ZfEvent],
    *,
...
def _admitted_closure_supersedes_legacy_escalation(
    events: list[ZfEvent],
    *,
    claim_payload: Mapping[str, Any],
    decision: Mapping[str, Any],
) -> bool:
    """Ignore only legacy worker escalation facts closed by exact Goal truth.

    An explicit human-decision contract remains blocking. Older workers emitted
    bare ``human.escalate`` events without a token or checkpoint; those facts
    otherwise survive forever even after the same generation and immutable
    candidate have passed Goal closure.
    """

    if str(claim_payload.get("claim_type") or "") != "admitted_goal_closure_result":
        return False
    source_event = _event_by_id(events, str(decision.get("event_id") or ""))
    if source_event is None or source_event.type not in {
        "human.escalate",
    ...
    if any(str(source_payload.get(key) or "").strip() for key in explicit_contract_fields):
        return False
    ...
    if not target_commit or not task_map_generation:
        return False
    ...
    for event in events:
        if event.id == source_event.id:
            seen_source = True
            continue
        if not seen_source or event.type not in {
            "flow.goal.closed",
        ...
        closure_generation = str(payload.get("task_map_generation") or "").strip()
        if (
            closure_target == target_commit
            and same_task_map_generation(closure_generation, task_map_generation)
        ):
            return True
    return False
...
def _completion_claim_invalid_reasons(
    ...
    *,
    ...
    if claim_type == "legacy_semantic_judge_verdict":
        if str(claim_payload.get("source_event_type") or "") != "judge.passed":
            return ["legacy_completion_source_not_judge"]
        source = _event_by_id(
            events,
            str(claim_payload.get("source_event_id") or ""),
        )
        if source is None:
            return ["legacy_completion_source_missing"]
        from zf.runtime.run_scope import (
            resolve_run_for_completion_cause,
            resolve_run_id,
        )

        source_run = resolve_run_for_completion_cause(events, source)
        claim_run = resolve_run_id(
            events,
            str(claim_payload.get("run_id") or ""),
        )
        if not source_run or not claim_run or source_run != claim_run:
            return ["legacy_completion_source_run_mismatch"]
        return []
    ...
    if not isinstance(admitted_ref, Mapping) or not str(admitted_ref.get("ref") or ""):
        invalid.append("missing_admitted_call_result_ref")
    if bool(claim_payload.get("product_acceptance_required")):
        for field in (
            ...
            if not str(claim_payload.get(field) or "").strip():
                invalid.append(f"missing_{field}")
        if str(claim_payload.get("product_acceptance_verdict") or "") != "passed":
            invalid.append("product_acceptance_not_passed")
    ...
    claim_product_required = bool(claim_payload.get("product_acceptance_required"))
    current_product_required = bool(current.get("product_acceptance_required"))
    if claim_product_required != current_product_required:
        invalid.append("stale_product_acceptance_required")
    if claim_product_required or current_product_required:
...
def run_goal_completion_event(
    ...
    if claim is None:
        return None
    return run_goal_completion_gate_event(
        [*events, cause, claim],
        claim=claim,
    )
...
def build_repair_ledger(events: list[ZfEvent]) -> dict[str, Any]:
    ...
    for event in events:
        ...
        elif event.type == "autoresearch.repair.spawned":
            row["status"] = "running"
            row["next_allowed_action"] = "wait_for_repair_terminal"
        elif event.type == "autoresearch.repair.terminated":
            terminal_status = str(payload.get("status") or "failed")
...
def build_repair_merge_queue(events: list[ZfEvent]) -> dict[str, Any]:
    ...
    for event in events:
        ...
        row = entries.setdefault(key, {
            ...
            if isinstance(payload.get("repair_contract_ref"), dict) else {},
            "repair_contract_digest": str(
                payload.get("repair_contract_digest") or ""
            ),
            "repair_contract_required": bool(
                payload.get("repair_contract_required", False)
            ),
...
def build_run_completion_profile(
    ...
    *,
    ...
    current_run_id = str(run_id or goal.get("run_id") or "").strip()
    if current_run_id:
        from zf.runtime.run_scope import events_for_run
...
def write_run_manager_projections(state_dir: Path, projection: dict[str, Any]) -> None:
    ...
    atomic_write_text(
        projections_dir / "run_continuation.json",
        json.dumps(
            projection.get("continuation", {}),
            ensure_ascii=False,
            indent=2,
...
def emit_human_escalation_package(
    ...
    *,
    ...
    workflow_run_id = str(
        action.get("workflow_run_id")
        or action.get("run_id")
        or action.get("pdd_id")
        or ""
    )
    run_id = str(action.get("run_id") or workflow_run_id)
    payload = {
        ...
        "failure_class": str(action.get("failure_class") or ""),
        "blocker_kind": str(action.get("blocker_kind") or ""),
        ...
        "integration_attempt_id": str(action.get("integration_attempt_id") or ""),
        ...
        "required_evidence_refs": _string_list(
            action.get("required_evidence_refs")
        ),
        "resume_condition": str(action.get("resume_condition") or ""),
        "suggested_options": _string_list(action.get("suggested_options"))
        or ["approve_controlled_action", "request_autoresearch", "safe_halt"],
        ...
        or "请选择是否允许 Run Manager 执行该高风险恢复动作。",
        "reason": reason,
    }
    escalate = writer.emit(
        ...
        correlation_id=workflow_run_id or run_id or None,
        payload=payload,
    )
    writer.emit(
        HUMAN_ESCALATION_SENT,
    ...
    for event in events:
        ...
        if route == "autoresearch":
            ...
            if _emit_autoresearch_request(
                ...
                if _autoresearch_inflight_request_ids(
                    writer.event_log.read_all()
                ):
                    status = "deferred"
                    reason = "autoresearch_inflight"
                else:
    ...
    for event in reversed(events):
        ...
        if str(payload.get("source_event_id") or "") == recommendation.id:
            if (
                ...
                in {
                    "autoresearch_inflight",
                    "autoresearch_request_already_exists",
                }
            ):
                return bool(_autoresearch_inflight_request_ids(events))
            return True
    return False
...
def _emit_autoresearch_request(
    *,
    ...
    if _request_id_seen(writer.event_log, RUN_MANAGER_AUTORESEARCH_REQUESTED, request_id):
        return False
    if _autoresearch_inflight_request_ids(writer.event_log.read_all()):
        return False
    try:
    ...
    payload = {
        ...
        "request_id": request_id,
        "operation_key": str(action.get("operation_key") or ""),
...
def _transition_name(
    *,
    ...
    if isinstance(completion, dict) and completion.get("status") == "blocked":
        blocking_human = completion.get("blocking_human_decisions")
        blocking_human = blocking_human if isinstance(blocking_human, list) else []
        if (
            str((projection.get("goal") or {}).get("status") or "") == "active"
            and any(
                _is_actionable_human_decision(ref)
                for ref in blocking_human
            )
        ):
            # The first blocked action already emitted escalate_human.  While
            # that recoverable gate remains open, later ticks only wait; they
            # must not turn an active run into a terminal projection or spam
            # duplicate escalation transitions.
...
def _execute_orchestrator_triage_request(
    *,
    ...
    request_type = semantic_failure_request_type(
        config,
    ...
    requested = writer.emit(
        ...
        payload={
            ...
            "request_kind": str(action.get("request_kind") or ""),
            ...
            "recovery_scope": str(action.get("recovery_scope") or "task"),
            "pdd_id": str(action.get("pdd_id") or ""),
            "trace_id": str(action.get("trace_id") or ""),
            "owner_route": "run_manager",
            "apply_policy": "proposal_only",
    ...
    *,
    ...
    request = writer.emit(
        ...
        payload={
            ...
            "continuation_commit": str(action.get("continuation_commit") or ""),
            "continuation_ref": str(action.get("continuation_ref") or ""),
            "continuation_source_event_id": str(
                action.get("continuation_source_event_id") or ""
            ),
            "partial_head_commit": str(action.get("partial_head_commit") or ""),
            "partial_source_branch": str(
                action.get("partial_source_branch") or ""
            ),
            "partial_worktree_clean": action.get("partial_worktree_clean"),
            "partial_dirty_files": _string_list(action.get("partial_dirty_files")),
            "previous_candidate_ref": str(
                action.get("previous_candidate_ref") or ""
            ),
            "previous_candidate_head_commit": str(
                action.get("previous_candidate_head_commit") or ""
            ),
    ...
    *,
    ...
    requested = writer.emit(
        ...
        payload={
            ...
            "lease_id": str(action.get("lease_id") or ""),
            "dispatch_id": str(action.get("dispatch_id") or ""),
            "identity_version": str(action.get("identity_version") or ""),
            "placement_epoch": int(action.get("placement_epoch") or 0),
            "recovery_decision_owner": "run_manager",
            "recovery_effect_owner": str(
                action.get("recovery_effect_owner")
                or "workflow_runtime_coordinator"
            ),
        },
    )
    outcome = writer.emit(
...
def _execute_repair_closeout_validate(
    *,
    ...
    contract_digest = str(action.get("repair_contract_digest") or "")
    if bool(action.get("repair_contract_required", False)):
        try:
            validate_repair_contract_action(state_dir, action)
            ...
            result = {
                ...
                "status": "failed",
                "reason": exc.code,
                "worktree": str(action.get("worktree_path") or ""),
                "results": [],
            ...
            result = execute_repair_verification_plan(
                worktree=str(
                    action.get("worktree_path") or action.get("worktree") or ""
                ),
                verification_plan=list(action.get("verification_plan") or []),
            )
            result["repair_contract_digest"] = contract_digest
    else:
        result = execute_repair_verification_plan(
    ...
    *,
    ...
    if bool(action.get("repair_contract_required", False)):
        try:
            ...
            return "failed"
        validation_result = action.get("validation_result")
        validation_result = (
            validation_result if isinstance(validation_result, dict) else {}
        )
        if str(validation_result.get("repair_contract_digest") or "") != str(
            action.get("repair_contract_digest") or ""
        ):
            writer.emit(
                RUN_MANAGER_ACTION_FAILED,
                actor="run-manager",
                ...
                payload={
                    ...
                    **_action_payload(action),
                    "status": "failed",
                    "reason": "repair_validation_contract_mismatch",
                },
            )
            return "failed"
    queue_payload = {
    ...
    if not bool(result.get("ok")):
        blocked = str(result.get("status") or "") == "blocked"
        review = writer.emit(
            RUN_MANAGER_REPAIR_MERGE_NEEDS_REVIEW,
            actor="run-manager",
        ...
        writer.emit(
            RUN_MANAGER_ACTION_BLOCKED if blocked else RUN_MANAGER_ACTION_FAILED,
            actor="run-manager",
    ...
    if passed and (effect_expected or effect_sequence):
        writer.emit(
            ...
            payload={
                ...
                **_action_payload(action),
                "effect_id": _action_effect_id(action),
...
def _action_effect_id(action: Mapping[str, Any]) -> str:
    checkpoint = str(action.get("checkpoint_id") or action.get("operation_key") or "")
    raw = "|".join((
        checkpoint,
        str(action.get("fanout_id") or ""),
        str(action.get("integration_attempt_id") or ""),
    ))
    return "action-effect-" + hashlib.sha1(raw.encode("utf-8")).hexdigest()[:16]


def _settle_pending_action_effects(
    events: list[ZfEvent],
    writer: EventWriter,
) -> int:
    terminal_by_id = {
        str((event.payload or {}).get("effect_id") or "")
        for event in events
        if event.type in {
            RUN_MANAGER_ACTION_EFFECT_PASSED,
            RUN_MANAGER_ACTION_EFFECT_FAILED,
    ...
    for index, pending in enumerate(events):
        if pending.type != RUN_MANAGER_ACTION_EFFECT_PENDING:
            continue
        ...
        if not effect_id or effect_id in terminal_by_id:
            continue
        recovery_event_type = {
            "goal_closure_identity_invalid": "flow.goal.closed",
            "goal_closure_aggregate_schema_failed": "goal.closure.synthesized",
        }.get(str(payload.get("failure_class") or ""))
        if (
            str(payload.get("action") or "") == "fanout-aggregate-rebuild"
            and recovery_event_type
        ):
            observed = next(
                (
                    candidate
                    for candidate in events[index + 1:]
                    if candidate.type == recovery_event_type
                ...
                writer.emit(
                    ...
                    payload={
                        ...
                        **_action_payload(payload),
                        ...
                        "status": "passed",
                        "reason": "Goal closure aggregate recovery observed",
                    },
                )
        ...
        writer.emit(
            RUN_MANAGER_ACTION_EFFECT_PASSED if passed else RUN_MANAGER_ACTION_EFFECT_FAILED,
            actor="run-manager",
            causation_id=observed.id,
            correlation_id=observed.correlation_id or pending.correlation_id,
            payload={
                ...
                **_action_payload(payload),
                ...
                "status": "passed" if passed else "failed",
                "reason": (
                    "candidate integration effect observed"
                    if passed else "candidate integration recovery did not make progress"
                ),
            },
        )
        terminal_by_id.add(effect_id)
...
def _effect_event_matches_scope(
    ...
    body = event.payload if isinstance(event.payload, dict) else {}
    scope_keys = ["workflow_run_id", "run_id", "fanout_id"]
    if (
        str(payload.get("action") or "") == "candidate-rework-apply"
        and str(payload.get("candidate_rework_action") or "") == "retrigger"
    ):
        # The action fanout is the rejected source; writer admission mints a
        # new fanout identity for the retriggered Task Map.
        scope_keys.remove("fanout_id")
...
def _pending_action_matches_run(
    action: dict[str, Any],
    *,
    events: list[ZfEvent],
    run_id: str,
) -> bool:
    """Keep unscoped legacy actions, but exclude proven older-run work."""

    from zf.runtime.run_scope import resolve_run_for_event, resolve_run_id

    for key in ("workflow_run_id", "run_id", "trace_id"):
        resolved = resolve_run_id(events, str(action.get(key) or ""))
        if resolved:
            return resolved == run_id
    event_ids = [
        str(action.get("source_event_id") or ""),
        str(action.get("blocking_event_id") or ""),
        *(
            str(value or "")
            for value in action.get("source_event_ids") or []
        ),
    ]
    wanted = {value for value in event_ids if value}
    for event in events:
        if event.id not in wanted:
            continue
        resolved = resolve_run_for_event(events, event)
        if resolved:
            return resolved == run_id
    return True
...
def _pending_repair_validation_actions(
    ...
    for item in items if isinstance(items, list) else []:
        ...
        action = {
            ...
            if isinstance(item.get("repair_contract_ref"), dict) else {},
            "repair_contract_digest": str(
                item.get("repair_contract_digest") or ""
            ),
            "repair_contract_required": bool(
                item.get("repair_contract_required", False)
            ),
    ...
    *,
    ...
    for item in items if isinstance(items, list) else []:
        ...
        action = {
            ...
            if isinstance(item.get("verification_plan"), list) else [],
            "repair_contract_ref": item.get("repair_contract_ref")
            if isinstance(item.get("repair_contract_ref"), dict) else {},
            "repair_contract_digest": str(
                item.get("repair_contract_digest") or ""
...
_INFRA_ACTORS = frozenset({
    ...
    "zf-autoresearch", "zf-autoresearch-resident", "operator", "",
})
...
def _pending_failure_closeout_activation_actions(events: list[ZfEvent]) -> list[dict[str, Any]]:
    if _current_run_completed_closeout(events) is not None:
        return []
    if _autoresearch_inflight_request_ids(events):
        return []
...
def _pending_semantic_event_actions(
    ...
    *,
    ...
    for idx, event in enumerate(events):
        ...
        if (
            event.type == "task.attempt.failed"
            and str(event_payload.get("identity_version") or "")
            == "operation-v2"
            and str(event_payload.get("recovery_owner") or "")
            == "run_manager"
            and event_payload.get("retryable") is True
        ):
            # The canonical TaskAttempt store owns the exact mechanical
            # recovery. A semantic diagnosis here would create a second owner.
            continue
        if (
        ...
        if (
            ...
            # threshold emits task.rework.capped. Candidate scope uses its
            # dedicated cap event and never becomes a task-local retry fact.
            continue
        ...
        if _is_blocked_goal_closure_schema_failure(event):
            if spec is None:
                continue
            action = _semantic_event_diagnostic_action(event, spec)
            blocked_payload = event_payload.get("blocked_event_payload")
            blocked_payload = (
                blocked_payload if isinstance(blocked_payload, dict) else {}
            )
            action.update({
                "action": "fanout-aggregate-rebuild",
                "safe_resume_action": "fanout_aggregate_rebuild",
                "failure_class": "goal_closure_aggregate_schema_failed",
                "action_policy": "auto_decide",
                "intervention_class": "auto_recover",
                "summary": (
                    "Rebuild the blocked goal closure aggregate from its "
                    "admitted child control result."
                ),
                "source_event_id": event.id,
                "aggregate_source_event_id": str(
                    event_payload.get("blocked_event_id") or ""
                ),
                "fanout_id": str(
                    event_payload.get("fanout_id")
                    or blocked_payload.get("fanout_id")
                    or ""
                ),
                "workflow_run_id": str(
                    event_payload.get("workflow_run_id")
                    or blocked_payload.get("workflow_run_id")
                    or event.correlation_id
                    or ""
                ),
                "goal_id": str(
                    event_payload.get("goal_id")
                    or blocked_payload.get("goal_id")
                    or ""
                ),
                "expected_downstream_events": [
                ...
                "verify_condition": (
                    "expected_downstream_event:goal.closure.synthesized"
                ),
                "effect_expected_events": ["goal.closure.synthesized"],
                "effect_failure_events": [
                    "goal.closure.synthesis.failed",
                    "discriminator.failed",
                ],
                "suggested_action": {
                    "kind": "fanout-aggregate-rebuild",
                    "event_type": event.type,
                    "scope": _semantic_event_scope(event, event_payload),
                },
            })
        elif event.type in RUN_MANAGER_PENDING_EVENT_TYPES:
...
def _is_blocked_goal_closure_schema_failure(event: ZfEvent) -> bool:
    if event.type != "discriminator.failed":
        return False
    payload = event.payload if isinstance(event.payload, dict) else {}
    blocked_payload = payload.get("blocked_event_payload")
    return (
        str(payload.get("blocked_event_type") or "")
        == "goal.closure.synthesized"
        and isinstance(blocked_payload, dict)
        and bool(
            str(payload.get("fanout_id") or blocked_payload.get("fanout_id") or "")
        )
        and "EventSchemaD" in {
            str(item) for item in payload.get("failed_d", [])
        }
    )
...
def _is_unknown_actionable_workflow_event(event: ZfEvent) -> bool:
    ...
    if not looks_actionable_event(event_type):
        return False
    if _is_generic_stage_replan_event(event):
        return False
...
def _is_generic_stage_replan_event(event: ZfEvent) -> bool:
    """Recognize a compiled Generic stage failure owned by the Kernel replan."""

    payload = event.payload if isinstance(event.payload, dict) else {}
    stage_id = str(payload.get("stage_id") or "").strip()
    digest = str(payload.get("generic_workflow_contract_digest") or "").strip()
    return bool(
        str(payload.get("flow_kind") or "").strip().lower() == "workflow"
        and stage_id
        and event.type == f"{stage_id}.failed"
        and len(digest) == 64
    ...
    *,
    ...
    if str(no_progress.get("status") if isinstance(no_progress, dict) else "") == "tripped":
        ...
        return _unknown_gap_item(
            ...
            summary=(
                "The no-progress projection tripped for a repeated failure "
                "fingerprint; diagnosis takes precedence over another "
                "mechanically valid but ineffective recovery action."
            ),
            source_event_ids=source_event_ids,
            source_ref=(
...
def _legacy_candidate_replan_fallbacks(
    ...
    *,
    ...
    """Prefer an anchored legacy replan when the semantic route is unavailable."""

    unavailable_by_source = {
        str(action.get("source_event_id") or ""): str(
            action.get("task_id") or ""
        )
    ...
    for action in candidate_actions:
        source_event_id = str(action.get("source_event_id") or "")
        if (
            source_event_id not in unavailable_by_source
            or str(action.get("candidate_rework_action") or "") != "replan"
            or not _legacy_replan_trigger_is_anchored(
                action,
                config=config,
                events=events,
            )
            ...
            continue
        sources.add(source_event_id)
        task_id = unavailable_by_source[source_event_id]
        if task_id:
            tasks.add(task_id)
    return sources, tasks


def _legacy_replan_trigger_is_anchored(
    action: dict[str, Any],
    *,
    config: ZfConfig,
    events: list[ZfEvent],
) -> bool:
    replan = getattr(config.workflow, "admission_replan", None)
    trigger = str(getattr(replan, "resynth_trigger", "") or "").strip()
    if not getattr(replan, "enabled", False) or not trigger:
        return False
    trace_id = str(action.get("trace_id") or "").strip()
    pdd_id = str(action.get("pdd_id") or action.get("feature_id") or "").strip()
    if not trace_id and not pdd_id:
        return False
    for event in reversed(events):
...
def _pending_candidate_recovery_actions(
    state_dir: Path,
    config: ZfConfig,
    events: list[ZfEvent],
    *,
    ...
    """Return candidate actions across both bounded retry and semantic triage.

    Once a candidate retry cap is recorded, the original rework action is
    complete. The idle sweep must still wake Run Manager for the candidate-scope
    triage request and for the controlled application of recorded advice.
    """

    actions = _pending_candidate_rework_actions(
        state_dir,
        config,
        events,
        project_root=project_root,
    ...
    resident_agent = build_resident_agent_projection(events, config=config)
    triage_actions = pending_rework_triage_actions(
        events,
        threshold=threshold,
        stale_seconds=_DIAGNOSIS_REQUEST_STALE_SECONDS,
        advisor_available=any(
    ...
    actions.extend(
        ...
        for action in triage_actions
        if (
            str(action.get("recovery_scope") or "") == "candidate"
            or str(action.get("source_event_type") or "")
            == "candidate.rework.capped"
            or str(action.get("failure_class") or "").startswith("candidate_rework_")
        )
    )
    return actions
    ...
    *,
    ...
    for item in items if isinstance(items, list) else []:
        ...
        if str(item.get("status") or "open") not in {"", "open", "unacknowledged"}:
            continue
        if _attention_is_informational(item):
            continue
...
def _attention_is_informational(item: dict[str, Any]) -> bool:
    envelope = item.get("problem_envelope")
    envelope = envelope if isinstance(envelope, dict) else {}
    action_policy = str(
        item.get("action_policy") or envelope.get("action_policy") or ""
    ).strip()
    intervention = str(
        item.get("intervention_class")
        or envelope.get("intervention_class")
        or ""
    ).strip()
    recovery_policy = str(
        item.get("recovery_policy") or envelope.get("recovery_policy") or ""
    ).strip()
    return (
        action_policy == "informational"
        or (intervention == "observe" and recovery_policy == "none")
    )
...
def _candidate_rework_action_from_plan(
    ...
    *,
    ...
    prepared_scope = prepare_candidate_rework_scope(
        state_dir=state_dir,
        ...
        anchor=anchor,
        rework_paths=list(getattr(plan, "rework_paths", ()) or ()),
        failed_task_ids=list(getattr(plan, "failed_task_ids", ()) or ()),
        rework_summary=dict(getattr(plan, "rework_summary", {}) or {}),
    )
    anchor = prepared_scope["anchor"]
    rework_action = str(getattr(plan, "action", "") or "")
    failed_task_ids = prepared_scope["failed_task_ids"]
    rework_paths = list(getattr(plan, "rework_paths", ()) or ())
    rework_summary = prepared_scope["rework_summary"]
    if prepared_scope.get("requires_replan") and rework_action == "retrigger":
        rework_action = "replan"
    source_event_type = str(getattr(plan, "source_event_type", "") or "")
    source_event_id = str(getattr(plan, "source_event_id", "") or "")
    source_event = next(
        (event for event in reversed(events) if event.id == source_event_id),
        None,
    )
    source_payload = (
        source_event.payload
    ...
    failed_candidate_head = str(
        source_payload.get("candidate_head_commit")
        or source_payload.get("commit")
        or ""
    ).strip()
    if failed_candidate_head:
        anchor["candidate_head_commit"] = failed_candidate_head
        if rework_action == "replan":
            # Candidate quality failures reject the verdict, not the assembled
            # product lineage. Replan from the immutable failed candidate so a
            # narrow repair cannot discard already integrated task refs.
            anchor["target_ref"] = failed_candidate_head
    failed_candidate_ref = str(source_payload.get("candidate_ref") or "").strip()
    if failed_candidate_ref:
        anchor["candidate_ref"] = failed_candidate_ref
    rework_feedback = list(getattr(plan, "feedback", ()) or ())
    required_actions: list[str] = []
    if source_event_type == "plan.rejected":
        from zf.runtime.candidate_rework_evidence import (
            plan_rejection_feedback,
            plan_rejection_required_actions,
        )

        rework_feedback = list(dict.fromkeys([
    ...
    failure_fingerprint = str(
        getattr(plan, "failure_fingerprint", "")
        or source_payload.get("failure_fingerprint")
        or source_payload.get("fingerprint")
        or ""
    )
    ...
    if failure_fingerprint:
        source_event_ids = [
            ...
            and str(event.payload.get("failure_fingerprint") or "")
            == failure_fingerprint
        ] or source_event_ids
    candidate_retry_mode = ""
    if (
        rework_action == "retrigger"
        and source_event_type == "integration.failed"
        and str(anchor.get("fanout_id") or "")
        and not failed_task_ids
        and not rework_summary.get("gap_tasks")
    ):
        candidate_retry_mode = "integration_only"
    expected = _candidate_rework_expected_events(
        rework_action,
        candidate_retry_mode=candidate_retry_mode,
    )
    ...
    action = {
        ...
        "candidate_ref": str(anchor.get("candidate_ref") or ""),
        "candidate_head_commit": str(anchor.get("candidate_head_commit") or ""),
    ...
    if candidate_retry_mode == "integration_only":
        action.update({
            ...
            "effect_verify_condition": "candidate integration reaches a new terminal",
        })
    elif rework_action == "retrigger":
        action.update({
            "effect_expected_events": [
                "fanout.started",
            ...
            "effect_verify_condition": (
                "retriggered Task Map reaches writer admission"
            ),
        })
    return action
...
def _candidate_rework_checkpoint_id(plan: object) -> str:
    continuation_failures = tuple(
        getattr(plan, "continuation_failure_ids", ()) or ()
    )
    raw = "|".join([
        "candidate-rework",
        ...
        # Admission failures need one fresh retry identity, but raw event ids
        # must not mint an unbounded checkpoint series for the same rejection.
        str(min(len(continuation_failures), 2)),
    ])
    return "crw-" + hashlib.sha1(raw.encode("utf-8")).hexdigest()[:16]


def _candidate_rework_expected_events(
    action: str,
    *,
    candidate_retry_mode: str = "",
) -> set[str]:
    if action == "retrigger":
        if candidate_retry_mode == "integration_only":
    ...
    if action == "escalate":
        return {"candidate.rework.capped"}
...
def _candidate_rework_shadowed_by_workflow(
    ...
    for workflow_action in workflow_actions:
        ...
        if _runtime_candidate_reemit_matches(workflow_action, action):
            return True
        if safe_action != "repair_failed_children":
            continue
        if not source_id:
            continue
        if source_id in {
...
def _workflow_reemit_shadowed_by_candidate(
    workflow_action: dict[str, Any],
    candidate_actions: list[dict[str, Any]],
) -> bool:
    if str(workflow_action.get("safe_resume_action") or "") != "reemit_candidate_ready":
        return False
    workflow_sources = {
        str(value)
        for value in workflow_action.get("source_event_ids") or []
        if str(value).strip()
    }
    for candidate in candidate_actions:
        if _runtime_candidate_reemit_matches(workflow_action, candidate):
            continue
        candidate_sources = {
            str(value)
        ...
        if workflow_sources.intersection(candidate_sources):
            return True
        for scope_key in ("pdd_id", "feature_id", "fanout_id"):
            workflow_scope = str(workflow_action.get(scope_key) or "")
            candidate_scope = str(candidate.get(scope_key) or "")
            if workflow_scope and workflow_scope == candidate_scope:
                return True
    return False


def _runtime_candidate_reemit_matches(
    workflow_action: dict[str, Any],
    candidate_action: dict[str, Any],
) -> bool:
    if (
        str(workflow_action.get("safe_resume_action") or "")
        != "reemit_candidate_ready"
        or str(workflow_action.get("source_event_type") or "")
        != "fanout.aggregate.completed"
    ):
        return False
    workflow_fanout = str(workflow_action.get("fanout_id") or "")
    candidate_fanout = str(candidate_action.get("fanout_id") or "")
    if workflow_fanout and candidate_fanout:
        return workflow_fanout == candidate_fanout
    workflow_pdd = str(
        workflow_action.get("pdd_id")
        or workflow_action.get("feature_id")
        or ""
    )
    candidate_pdd = str(
        candidate_action.get("pdd_id")
        or candidate_action.get("feature_id")
        or ""
    )
    return bool(workflow_pdd and workflow_pdd == candidate_pdd)
...
def _candidate_actions_are_human_escalations(actions: list[dict[str, Any]]) -> bool:
    ...
    for action in actions:
        ...
        decision = decision if isinstance(decision, dict) else {}
        if (
            str(action.get("candidate_rework_action") or "") == "escalate"
            and not bool(action.get("semantic_triage_pending"))
        ):
            continue
...
def _action_payload(action: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "fingerprint": str(action.get("fingerprint") or ""),
        "recovery_case_id": str(action.get("recovery_case_id") or ""),
        "recovery_action_family": str(action.get("recovery_action_family") or ""),
        "recovery_admission": (
            action.get("recovery_admission")
            if isinstance(action.get("recovery_admission"), dict)
            else {}
        ),
        "attempt": _safe_int(action.get("attempt")),
        "request_id": str(action.get("request_id") or ""),
        "operation_key": str(action.get("operation_key") or ""),
        "operation_precondition": (
            action.get("operation_precondition")
            if isinstance(action.get("operation_precondition"), dict)
            else {}
        ),
        "operation_attempt_cap": int(action.get("operation_attempt_cap") or 0),
        "operation_deadline": action.get("operation_deadline") or {},
        "operation_terminal_fallback": str(
            action.get("operation_terminal_fallback") or ""
        ),
        "role": str(action.get("role") or ""),
        "failure_count": int(action.get("failure_count") or 0),
        "failure_event_ids": _string_list(action.get("failure_event_ids")),
        ...
        "repair_contract_ref": action.get("repair_contract_ref")
        if isinstance(action.get("repair_contract_ref"), dict) else {},
        "repair_contract_digest": str(
            action.get("repair_contract_digest") or ""
        ),
        "repair_contract_required": bool(
            action.get("repair_contract_required", False)
        ),
        "risk_classification": action.get("risk_classification")
        if isinstance(action.get("risk_classification"), dict) else {},
        ...
        if lease_key:
            ...
            if previous and previous in out:
                ...
                out[previous] = {
                    **existing,
                    **payload,
                    "event_id": str(existing.get("event_id") or event.id),
                    "event_type": str(existing.get("event_type") or event.type),
                    "created_ts": str(existing.get("created_ts") or event.ts),
...
def _is_actionable_human_decision(ref: Any) -> bool:
    """Return whether a blocking notice carries an explicit resume contract."""

    if not isinstance(ref, dict) or not _is_blocking_human_decision(ref):
        return False
    return any(
...
def _event_superseded_by_later_progress(
    ...
    for event in events:
        ...
        if not seen_source:
            continue
        if source_event.type == "run.goal.blocked" and event.type == "run.goal.updated":
            update_payload = event.payload if isinstance(event.payload, dict) else {}
            update_run_id = str(
                update_payload.get("run_id")
                or update_payload.get("workflow_run_id")
                or event.correlation_id
                or ""
            )
            if (
                source_run_id
                and update_run_id == source_run_id
                and str(update_payload.get("status") or "").strip().lower()
                in {"active", "running"}
            ):
                return True
...
def _outcome_no_progress_break(events: list[ZfEvent], action: dict[str, Any]) -> str:
    """Break repeated task or batch recovery that produces no milestone."""

    scope = _run_action_scope(action)
    safe_action = str(action.get("safe_resume_action") or "")
    if not scope or not safe_action:
        return ""
    ...
    for idx, event in enumerate(events):
        ...
        if str(payload.get("safe_resume_action") or "") != safe_action:
            continue
        if not _event_matches_run_action_scope(event, scope):
            continue
    ...
    for event in events[window_start + 1:]:
        if event.type not in _OUTCOME_PROGRESS_EVENTS:
            continue
        if _event_matches_run_action_scope(event, scope):
            return ""
    scope_text = ",".join(f"{key}={value}" for key, value in scope)
    return (
        f"safe_resume_action {safe_action!r} applied "
        f"{len(applied_idx)} times for {scope_text} without progress"
    )


def _run_action_scope(action: dict[str, Any]) -> tuple[tuple[str, str], ...]:
...
def _event_matches_run_action_scope(
    event: ZfEvent,
    ...
    for key, expected in scope:
        ...
        if key == "task_id":
            actual = str(event.task_id or actual)
        if actual:
            return actual == expected
    return False


def _emit_no_progress_run_terminal(
    writer: EventWriter,
    *,
    action: dict[str, Any],
    reason: str,
    ...
    try:
        ...
        events = []
    action_run_id = str(
        action.get("workflow_run_id") or action.get("run_id") or ""
    ).strip()
    goal = build_run_goal_projection(events, run_id=action_run_id)
    run_id = str(action_run_id or goal.get("run_id") or "").strip()
    if not run_id or str(goal.get("status") or "") in {"complete", "blocked"}:
        return None
    for event in events:
    ...
    fingerprint_raw = "|".join([
        run_id,
        str(action.get("safe_resume_action") or ""),
        *(f"{key}={value}" for key, value in scope),
    ])
    ...
    evidence_event_ids = [
        ...
        for event in events
        if event.type == "workflow.resume.applied"
        and _event_matches_run_action_scope(event, scope)
    ][-3:]
    if causation_id:
        evidence_event_ids.append(causation_id)
    return writer.emit(
        "run.goal.blocked",
        actor="run-manager",
        causation_id=causation_id,
...
def _action_seen(events: list[ZfEvent], action: dict[str, Any]) -> bool:
    checkpoint_id = str(action.get("checkpoint_id") or "")
    operation_key = str(action.get("operation_key") or "")
    if not checkpoint_id and not operation_key:
        return False
    for event in events:
        payload = event.payload if isinstance(event.payload, dict) else {}
        if (
            operation_key
            and str(payload.get("operation_key") or "") == operation_key
            and event.type in {
                RUN_MANAGER_ACTION_APPLIED,
                RUN_MANAGER_ACTION_BLOCKED,
                RUN_MANAGER_ACTION_FAILED,
...
def _operation_precondition_holds(
    events: list[ZfEvent],
    action: dict[str, Any],
) -> bool:
    precondition = action.get("operation_precondition")
    if not isinstance(precondition, dict) or not precondition:
        return True
    expected_run_id = str(precondition.get("run_id") or "")
    goal = build_run_goal_projection(events, run_id=expected_run_id)
    current = build_run_continuation_projection(
        events,
        goal=goal,
        pending_actions=[],
        completion_profile={},
    )
    for key in ("run_id", "generation", "progress_digest"):
        expected = str(precondition.get(key) or "")
        if expected and str(current.get(key) or "") != expected:
            return False
    return not bool(current.get("terminal"))
...
def _derive_phase(events: list[ZfEvent]) -> str:
    for event in reversed(events):
        etype = event.type
        payload = event.payload if isinstance(event.payload, dict) else {}
        stage_phase = _phase_from_stage_identity(
            str(payload.get("stage_id") or payload.get("producer_stage_id") or "")
        )
        if stage_phase and etype in {
            "fanout.started",
            "fanout.child.dispatched",
...
def _phase_from_stage_identity(stage_id: str) -> str:
    identity = stage_id.strip().lower()
    if not identity:
        return ""
    if "scan" in identity:
        return "scan"
    if "plan" in identity:
        return "plan"
    if "verify" in identity or "test" in identity:
        return "verify"
    if "review" in identity or "critic" in identity:
        return "review"
    if any(token in identity for token in ("judge", "closure", "final")):
        return "judge"
    if any(token in identity for token in ("impl", "dev", "writer", "rework")):
        return "impl"
    return ""
...
def _next_wait(
    ...
    *,
    ...
    completion_profile = (
        completion_profile if isinstance(completion_profile, dict) else {}
    )
    blocking_human = completion_profile.get("blocking_human_decisions")
    blocking_human = blocking_human if isinstance(blocking_human, list) else []
    if state == "needs_human" and any(
        _is_actionable_human_decision(ref)
        for ref in blocking_human
    ):
...
def _status_explain_decision(
    *,
    ...
    pending_human = pending_human if isinstance(pending_human, list) else []
    run_refs = [
        ref for ref in pending_human
        if isinstance(ref, dict) and _is_blocking_human_decision(ref)
    ]
    actionable_run_refs = [
        ref for ref in run_refs
        if _is_actionable_human_decision(ref)
    ]
    side_refs = [
        ref for ref in pending_human
        if isinstance(ref, dict) and not _is_blocking_human_decision(ref)
    ]
    if actionable_run_refs:
        return {
            "owner_route": "human",
            "intervention_class": "human_decision",
            ...
            "blocking_refs": [
                *actionable_run_refs,
                *[ref for ref in run_refs if ref not in actionable_run_refs],
            ],
...
def _autoresearch_inflight_request_ids(events: list[ZfEvent]) -> set[str]:
    """Return diagnosis requests that do not have a bounded terminal answer."""
    ...
    request_by_checkpoint: dict[str, str] = {}
    for event in events:
        payload = event.payload if isinstance(event.payload, dict) else {}
        if event.type in request_types:
            request_id = _autoresearch_result_request_id(event, payload)
            if request_id:
                requested.add(request_id)
                checkpoint_id = str(payload.get("checkpoint_id") or "")
                if checkpoint_id:
                    request_by_checkpoint[checkpoint_id] = request_id
        elif event.type in terminal_types:
            request_id = _autoresearch_result_request_id(event, payload)
            if request_id:
                terminal.add(request_id)
        elif event.type in {RUN_MANAGER_ACTION_FAILED, RUN_MANAGER_ACTION_BLOCKED}:
            request_id = request_by_checkpoint.get(
                str(payload.get("checkpoint_id") or "")
            )
            if request_id:
                terminal.add(request_id)
