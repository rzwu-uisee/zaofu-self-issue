...
4. Verify:
   - run `python scripts/dev-verify.py plan --base dev` from the source
     worktree, inspect the changed-file/domain/reason output, then run the
     matching `python scripts/dev-verify.py run --base dev`;
   - if an approved source change is intentionally outside the known map,
     pass exact `--tests <path>` values and record why they close the impact;
     never weaken the selector or treat an unknown source path as no-test;
