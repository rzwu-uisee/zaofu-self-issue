...
2. Read and validate the active route:

   ```bash
   zf workflow routes --task TASK-ID --format json
   ```

   Select `route_id=research:fixed` only when it is present and available.
   The catalog owns the internal pattern, topology, and roles.
...
4. Create the exact proposal without requiring Web:
...
zf workflow start \
  ...
  --objective "<research topic>" \
  --parameters-json '{"topic":"<research topic>","expected_output":"research synthesis plus PRD/refactor prompt inputs"}' \
  --propose \
  --format json
```
