...
def test_batch_resume_completed_dependencies_dispatch_requested_child(
    ...
    _start(orch, payload={
        ...
        "rework_of": "failed-before-start",
        "rework_attempt": 1,
    })

    started = next(event for event in log.read_all() if event.type == "fanout.started")
    assert started.payload["trigger_payload"]["completed_task_ids"] == ["TASK-1"]
    manifest = _manifest(state_dir, started.payload["fanout_id"])
    assert _child(manifest, "TASK-2")["status"] == "dispatched"
...
def test_batch_resume_completed_dependencies_unblock_slot_reconcile(
    tmp_path: Path,
    monkeypatch,
) -> None:
    state_dir, log, transport, orch = _state(
    ...
    monkeypatch.setattr(
        "zf.runtime.orchestrator_fanout._writer_task_dependencies_satisfied",
    ...
    fanout_id = _fanout_id(log)
    assert _child(_manifest(state_dir, fanout_id), "TASK-2")["status"] == "queued"
    monkeypatch.undo()

    orch.run_once(events=[])

    assert _child(_manifest(state_dir, fanout_id), "TASK-2")["status"] == "dispatched"
    assert [sent[0] for sent in transport.sent] == ["dev-1"]


def test_writer_preparation_failure_defers_without_terminal_child(
    tmp_path: Path,
    monkeypatch,
) -> None:
    state_dir, log, transport, orch = _state(
    ...
    original = WorkdirManager.sync_writer_to_source_ref
    attempts = {"count": 0}

    def fail_once(self, role, *, source_ref_override=""):  # noqa: ANN001
        attempts["count"] += 1
        if attempts["count"] == 1:
            raise RuntimeError("simulated readonly generated tree")
        return original(
            self,
            role,
    ...
    monkeypatch.setattr(WorkdirManager, "sync_writer_to_source_ref", fail_once)
    ...
    assert any(
        ...
        and event.payload.get("failure_kind") == "dispatch_preparation"
        for event in events
    )
    assert not [
    ...
    deferred_task_id = pending[0]["task_id"]
    deferred_task = TaskStore(state_dir / "kanban.json").get(deferred_task_id)
    assert deferred_task is not None
    assert deferred_task.status == "blocked"
    assert deferred_task.active_dispatch_id == ""

    orch.run_once(events=[])

    recovered = _child(_manifest(state_dir, fanout_id), deferred_task_id)
    assert recovered["status"] == "dispatched"
    assert deferred_task_id in {
...
def _start(
    ...
    *,
    ...
    trigger_payload = {"pdd_id": "F-11111111", **(payload or {})}
...
def test_collect_payload_list_normalizes_structured_refs() -> None:
    assert WriterFanoutDataMixin._collect_payload_list(
        [{
            "artifact_refs": [
                {"path": "artifacts/input.json", "sha256": "a" * 64},
...
def test_verified_generation_suppresses_recovery_writer_retrigger(
    tmp_path: Path,
) -> None:
    state_dir, log, _transport, orch = _state(tmp_path)
    _seed_tasks(state_dir)
    ...
    assert suppressed[0].payload["reason"] == "generation_already_verified"
    assert suppressed[0].payload["candidate_event_id"] == "candidate-ready"
    assert suppressed[0].payload["verification_event_id"] == "test-passed"


def test_recovery_cancels_active_writer_for_verified_generation(
    tmp_path: Path,
) -> None:
    state_dir, log, _transport, orch = _state(tmp_path)
    _seed_tasks(state_dir)
    ...
    writer.append(ZfEvent(
        ...
        payload={
            ...
            "task_ids": ["TASK-1", "TASK-2"],
            "task_map_generation": generation,
            "redispatch_fingerprint": "stall:run-1:dev-fanout",
        },
    ))
    fanout_id = "fanout-dev-fanout-recovery"
    writer.append(ZfEvent(
        id="fanout-started",
        type="fanout.started",
        correlation_id="run-1",
        causation_id="recovery-task-map",
        payload={
            "fanout_id": fanout_id,
            "stage_id": "dev-fanout",
            "topology": "fanout_writer_scoped",
            "trace_id": "run-1",
            "trigger_event_id": "recovery-task-map",
            "target_ref": "main",
            "task_ids": ["TASK-1", "TASK-2"],
        },
    ))
    ...
    assert (
        ...
        == "recovery_generation_already_verified"
    )
    assert cancelled[0].payload["superseded_by"] == "candidate-ready"
    ...
    payload = _FanoutPayloadProbe()._fanout_flow_identity_payload(
        {
            ...
            "trigger_payload": {
                ...
                "workflow_proposal_digest": "proposal-sha",
                "effective_config_ref": {
                    "ref": "artifacts/workflow/effective/req-001.json",
                    "sha256": "config-sha",
                },
...
def test_fanout_success_payload_preserves_generic_goal_identity() -> None:
    ...
    payload = _FanoutPayloadProbe()._fanout_flow_identity_payload(
        {
            "fanout_id": "fanout-generic-synthesize",
            "trigger_payload": {
                "request_id": "req-generic",
                "workflow_run_id": "req-generic",
                "flow_kind": "workflow",
                "goal_id": "req-generic",
    ...
    assert payload["goal_id"] == "req-generic"
    assert payload["workflow_generation"] == "a" * 64
    assert payload["workflow_intent"] == "research"
    assert payload["completion_profile"] == "artifact_delivery"
    assert payload["required_delivery_artifacts"][0]["source_ref"] == (
        "synthesize.report"
    )
    assert payload["goal_claim_set_ref"] == claim_ref
    assert payload["goal_claim_set_digest"] == "c" * 64
...
def test_fanout_success_payload_restores_frozen_identity_from_assignment() -> None:
    effective_ref = {
        "ref": "artifacts/workflow/effective/req-001.json",
        "sha256": "config-sha",
    }
    payload = _FanoutPayloadProbe()._fanout_flow_identity_payload(
        {
            ...
            "trigger_payload": {"workflow_run_id": "run-001"},
            "children": [{
                "child_id": "issue-verify",
                "effective_config_digest": "forged-child-config-sha",
                "payload": {
    ...
    assert payload["effective_config_ref"] == effective_ref
    assert payload["effective_config_digest"] == "config-sha"
    assert payload["run_contract_digest"] == "contract-sha"


def test_fanout_success_payload_restores_flow_identity_from_assignment() -> None:
    payload = _FanoutPayloadProbe()._fanout_flow_identity_payload(
        {
            "fanout_id": "fanout-prd",
            "children": [{
                "child_id": "prd-impl",
                "payload": {
                    "request_id": "req-prd",
    ...
    assert payload["flow_kind"] == "prd"
    assert payload["request_kind"] == "prd"
...
def test_writer_briefing_preserves_verified_partial_work_before_blocking(
    ...
    state_dir, _log, transport, orch = _state(
        tmp_path,
        synthesize_canonical=True,
    )
    ...
    briefing = transport.sent[0][1].read_text(encoding="utf-8")
    assert "preserve them before `dev.blocked`" in briefing
    assert "verify every remaining dirty path is inside `allowed_paths`" in briefing
    assert "stage only those paths with explicit pathspecs" in briefing
    assert "checkpoint is continuation evidence only" in briefing
    assert "not `dev.build.done`" in briefing
    assert "do not edit or commit again before submitting `dev.blocked`" in briefing
    assert "captures that exact HEAD, branch, and cleanliness" in briefing
...
def test_prd_task_map_owner_never_falls_back_to_issue_writer(tmp_path: Path):
    state_dir, _log, _transport, orch = _state(
        tmp_path,
        synthesize_canonical=True,
    )
    task_map = state_dir / "artifacts" / "F-11111111" / "task_map.json"
    task_map_data = json.loads(task_map.read_text(encoding="utf-8"))
    task_map_data["tasks"][0].update({
        "owner_role": "impl",
        "preferred_impl_role": "impl",
    })
    task_map.write_text(json.dumps(task_map_data), encoding="utf-8")
    orch.config.roles = [
        RoleConfig(
            name="issue-fix",
            instance_id="issue-fix-lane-0",
            backend="mock",
            role_kind="writer",
            flow_kind="issue",
        ),
    ...
    task = TaskStore(state_dir / "kanban.json").get("TASK-1")
    assert task is not None
    assert task.contract.owner_role == "prd-dev"
    assert task.contract.owner_instance == "prd-dev-lane-0"
    assert task.contract.evidence_contract["semantic_owner_role"] == "impl"
    assert task.contract.evidence_contract["flow_kind"] == "prd"
    assert task.contract.evidence_contract["workflow_run_id"] == "run-prd"
...
def test_refactor_replan_refreshes_existing_canonical_task_map_refs(tmp_path: Path):
        ...
        "tasks": [
            {
                ...
                "acceptance_criteria": [
                    {
                        "id": "AC-TASK-1",
                        "statement": "v2 behavior remains compatible",
    ...
    assert refreshed.contract.scope == ["a2.txt"]
    assert refreshed.contract.behavior == "Rewrite TASK-1 against v2 fixtures."
    assert [item["id"] for item in refreshed.contract.validation["commands"]] == [
        "verify-unit",
        "verify-e2e",
    ]
    assert refreshed.contract.acceptance_criteria == [{
        "id": "AC-TASK-1",
        "statement": "v2 behavior remains compatible",
        "verification_command_ids": ["verify-unit", "verify-e2e"],
    }]
    assert refreshed.contract.goal_claim_ids == ["GOAL-V2"]
    revision = effective_contract_revision(refreshed)
    task1_update = next(event for event in updates if event.task_id == "TASK-1")
    assert task1_update.payload["contract"] == asdict(refreshed.contract)

    # The next runtime tick replays task.contract.update through housekeeping.
    # That replay must not collapse a canonical multi-command contract into
    # the legacy single verification string.
    orch._apply_housekeeping(task1_update)  # type: ignore[attr-defined]
    replayed = store.get("TASK-1")
    assert replayed is not None
    assert effective_contract_revision(replayed) == revision
    assert replayed.contract.validation == refreshed.contract.validation
    assert replayed.contract.acceptance_criteria == refreshed.contract.acceptance_criteria
    assert replayed.contract.goal_claim_ids == refreshed.contract.goal_claim_ids
...
def test_replan_contract_refresh_accepts_feature_correction_in_same_workflow_run(
    tmp_path: Path,
):
    state_dir, _log, _transport, orch = _state(
        tmp_path,
    ...
    store.add(Task(
        ...
        status="in_progress",
        contract=TaskContract(
            feature_id="PDD-BOOTSTRAP",
            evidence_contract={
                "source": "refactor_task_map",
                "workflow_run_id": "RUN-1",
                "source_refs": {
                    "task_map_ref": ".zf/artifacts/old/task-map.json",
                },
            },
        ),
    ))
    loaded = SimpleNamespace(
        feature_id="PRODUCT-FEATURE",
        pdd_id="PDD-BOOTSTRAP",
        task_map_ref=".zf/artifacts/new/task-map.json",
        is_replan=True,
        workflow_run_id="RUN-1",
    )

    assert orch._can_refresh_writer_task(  # type: ignore[attr-defined]
        store.get("TASK-1"),
        loaded,
    )


def test_replan_contract_refresh_rejects_feature_change_across_workflow_runs(
    tmp_path: Path,
):
    state_dir, _log, _transport, orch = _state(
        tmp_path,
    ...
    task = Task(
        ...
        contract=TaskContract(
            ...
            evidence_contract={
                ...
                "workflow_run_id": "RUN-1",
                "source_refs": {
                    "task_map_ref": ".zf/artifacts/run-1/task-map.json",
                },
            },
        ),
    )
    loaded = SimpleNamespace(
        ...
        pdd_id="FEATURE-2",
        task_map_ref=".zf/artifacts/run-2/task-map.json",
        is_replan=True,
        workflow_run_id="RUN-2",
    )

    assert not orch._can_refresh_writer_task(  # type: ignore[attr-defined]
        task,
        loaded,
    )
...
def test_writer_fanout_starts_from_task_map_target_commit(tmp_path: Path):
    ...
    assert task_doc_manifest["assigned_to"] == first["role_instance"]
    assert task_doc_manifest["active_dispatch_id"] == first["run_id"]
    briefing = transport.sent[0][1].read_text(encoding="utf-8")
    ...
    assert candidate_manifest["requested_base_ref"] == target_commit
    assert candidate_manifest["base_commit"] == target_commit
    assert _git(
        tmp_path,
        "show",
        "candidate/F-11111111:baseline.txt",
    ) == "planned baseline"


def test_writer_fanout_task_local_base_overrides_stale_map_target(
    tmp_path: Path,
):
    state_dir, log, transport, orch = _state(tmp_path)
    _seed_tasks(state_dir, task_ids=("TASK-1",))
    stale_target = _git(tmp_path, "rev-parse", "HEAD")
    continuation_base = _commit(
        tmp_path,
        "continuation.txt",
        "verified continuation\n",
        "verified continuation",
    )
    _git(tmp_path, "reset", "--hard", stale_target)
    task_map = state_dir / "artifacts" / "F-11111111" / "task_map.json"
    payload = json.loads(task_map.read_text(encoding="utf-8"))
    payload["target_commit"] = stale_target
    payload["tasks"] = [payload["tasks"][0]]
    payload["tasks"][0].update({
        "base_commit": continuation_base,
        "source_refs": [f"git:{continuation_base}"],
    })
    task_map.write_text(json.dumps(payload), encoding="utf-8")
    ...
    assert manifest["target_ref"] == continuation_base
    assert child["target_ref"] == continuation_base
    assert child["payload"]["dispatch_base_commit"] == continuation_base
    assert dispatched.payload["target_ref"] == continuation_base
    assert dispatched.payload["base_commit"] == continuation_base
    assert _git(Path(child["workdir"]), "rev-parse", "HEAD") == continuation_base
    briefing = transport.sent[0][1].read_text(encoding="utf-8")
    assert f'"base_ref": "{continuation_base}"' in briefing
    assert f'"base_git_head": "{continuation_base}"' in briefing

    # A run created before this fix can still carry the stale fanout-level
    # target after restart. Candidate integration must recover the immutable
    # task baseline rather than rebuilding onto that stale root.
    manifest_path = state_dir / "fanouts" / manifest["fanout_id"] / "manifest.json"
    stale_manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    stale_manifest["target_ref"] = stale_target
    stale_manifest["children"][0]["target_ref"] = stale_target
    ...
    orch.run_once(events=[ZfEvent(
        ...
        payload={
            ...
            "run_id": child["run_id"],
            "pdd_id": "F-11111111",
            "base_git_head": continuation_base,
    ...
    assert candidate_manifest["requested_base_ref"] == continuation_base
    assert candidate_manifest["base_commit"] == continuation_base


def test_writer_preparation_repairs_stale_child_target_from_task_base(
    tmp_path: Path,
):
    ...
    context = FanoutContext.create(
        ...
        trigger_event_id="task-map-stale",
        target_ref=stale_target,
        role_instances=[],
    )
    child = FanoutChild(
        child_id="dev-1-TASK-1",
    ...
    prepared = orch._prepare_writer_fanout_child_operation(  # type: ignore[attr-defined]
        context=context,
        child=child,
        task_item=task_item,
        role=role,
        causation_id="fanout-started-stale",
    )

    assert task_item["dispatch_base_commit"] == continuation_base
    assert prepared["operation_payload"]["target_ref"] == continuation_base
    assert prepared["workdir_sync"]["source_ref"] == continuation_base
    assert _git(
        Path(prepared["plan"].project_path),
        "rev-parse",
        "HEAD",
    ) == continuation_base


def test_writer_fanout_rejects_conflicting_task_local_bases(tmp_path: Path):
    state_dir, log, _transport, orch = _state(tmp_path)
    _seed_tasks(state_dir)
    stale_target = _git(tmp_path, "rev-parse", "HEAD")
    first_base = _commit(tmp_path, "first.txt", "first\n", "first base")
    second_base = _commit(tmp_path, "second.txt", "second\n", "second base")
    _git(tmp_path, "reset", "--hard", stale_target)
    task_map = state_dir / "artifacts" / "F-11111111" / "task_map.json"
    payload = json.loads(task_map.read_text(encoding="utf-8"))
    payload["target_commit"] = stale_target
    payload["tasks"][0].update({
        ...
        "source_refs": [f"git:{first_base}"],
    })
    payload["tasks"][1].update({
        "base_commit": second_base,
        "source_refs": [f"git:{second_base}"],
    })
    task_map.write_text(json.dumps(payload), encoding="utf-8")

    _start(orch)

    assert not [event for event in log.read_all() if event.type == "fanout.started"]
    cancelled = [
        event for event in log.read_all() if event.type == "fanout.cancelled"
    ]
    assert len(cancelled) == 1
    assert "conflicting base_commit values" in cancelled[0].payload["reason"]


def test_recovered_writer_binding_refreshes_task_capsule(tmp_path: Path):
    state_dir, log, _transport, orch = _state(tmp_path)
    _seed_tasks(state_dir)
    _start(orch)
    first = _child(_manifest(state_dir, _fanout_id(log)), "TASK-1")
    store = TaskStore(state_dir / "kanban.json")
    manifest_path = state_dir / "task_docs" / "TASK-1" / "manifest.json"
    stale_manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    stale_manifest["contract_revision"] = "contract-stale"
    manifest_path.write_text(json.dumps(stale_manifest), encoding="utf-8")

    orch._recover_writer_fanout_task_bindings()  # type: ignore[attr-defined]

    recovered = store.get("TASK-1")
    assert recovered is not None
    assert recovered.status == "in_progress"
    assert recovered.assigned_to == first["role_instance"]
    assert recovered.active_dispatch_id == first["run_id"]
    refreshed_manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    assert refreshed_manifest["assigned_to"] == first["role_instance"]
    assert refreshed_manifest["active_dispatch_id"] == first["run_id"]
    assert (
        refreshed_manifest["contract_revision"]
        == recovered.contract.contract_revision
    )


def test_writer_fanout_starts_from_task_map_metadata_target_commit(tmp_path: Path):
    state_dir, log, transport, orch = _state(tmp_path)
    _seed_tasks(state_dir)
    main_commit = _git(tmp_path, "rev-parse", "HEAD")
    target_commit = _commit(
    ...
    task_map = state_dir / "artifacts" / "F-11111111" / "task_map.json"
    task_map_payload = json.loads(task_map.read_text(encoding="utf-8"))
    task_map_payload["metadata"] = {"target_commit": target_commit}
    task_map.write_text(json.dumps(task_map_payload), encoding="utf-8")
...
def test_writer_briefing_uses_canonical_contract_required_read(tmp_path: Path):
    ...
    _start(orch)
    manifest = _manifest(state_dir, _fanout_id(_log))
    first = _child(manifest, "TASK-1")
    task_doc_manifest = json.loads(
        (
            state_dir
    ...
    briefing = transport.sent[0][1].read_text(encoding="utf-8")
    assert (
        task_doc_manifest["contract_revision"]
        == first["contract_revision"]
    )
...
def test_writer_dispatch_fence_matches_terminal_dispatch_id_before_workflow_run_id(
    tmp_path: Path,
):
    state_dir, log, _transport, orch = _state(tmp_path)
    _seed_tasks(state_dir)
    ...
    dispatch_id = str(dispatch.payload["run_id"])

    assert orch._task_dispatch_id_is_active(  # type: ignore[attr-defined]
        "TASK-1",
        dispatch_id,
    )
    ...
    assert not orch._task_dispatch_id_is_active(  # type: ignore[attr-defined]
        "TASK-1",
        dispatch_id,
    )
...
def test_task_ref_repair_rotates_blocking_writer_operation(
    ...
    fanout_id = _fanout_id(log)
    manifest = _manifest(state_dir, fanout_id)
    original_child = _child(manifest, "TASK-1")
    stage_id = manifest["stage_id"]
    old_operation_id = original_child["operation_id"]
    old_request_hash = original_child["request_hash"]
    old_run_id = original_child["run_id"]
    ...
    source = writer.append(ZfEvent(
        type="dev.build.done",
        actor=original_child["role_instance"],
        task_id="TASK-1",
        correlation_id="trace-1",
        payload={
            "fanout_id": fanout_id,
            "stage_id": stage_id,
            "child_id": original_child["child_id"],
            "run_id": old_run_id,
            "role_instance": original_child["role_instance"],
            "source_commit": commit,
            "source_branch": original_child["source_branch"],
            "workdir": original_child["workdir"],
            "summary": "source completion with stale call identity",
            "operation_id": old_operation_id,
            "request_hash": old_request_hash,
            "attempt_id": original_child["attempt_id"],
            "result_protocol_mode": "blocking",
            "contract_revision": original_child["contract_revision"],
            "contract_snapshot_ref": original_child["contract_snapshot_ref"],
            "contract_snapshot_digest": original_child["contract_snapshot_digest"],
        },
    ))
    repair = writer.append(ZfEvent(
        ...
        payload={
            ...
            "reason": "stale task contract: rotate immutable operation",
        },
        causation_id=source.id,
    ))
    orch._last_worker_state[original_child["role_instance"]] = "blocked_human"  # type: ignore[attr-defined]

    dispatched_role = orch._dispatch_rework(  # type: ignore[attr-defined]
        store.get("TASK-1"),  # type: ignore[arg-type]
        repair,
    )
    ...
    new_request = next(
        ...
        if event.type == "workflow.operation.requested"
        and event.task_id == "TASK-1"
        and event.payload.get("operation_id") != old_operation_id
    )
    assert any(
    ...
    rebound = next(
        ...
        for event in reversed(events)
        if event.type == "fanout.child.dispatched"
        and event.payload.get("repair_of_event_id") == repair.id
    )
    assert rebound.payload["fanout_id"] == fanout_id
    assert rebound.payload["child_id"] == original_child["child_id"]
    assert rebound.payload["run_id"] != old_run_id
    assert rebound.payload["operation_id"] == new_request.payload["operation_id"]
    assert rebound.payload["attempt_id"] == repair_dispatch.payload["attempt_id"]
    assert rebound.payload["contract_revision"] != original_child["contract_revision"]
    assert "Runtime Durable Repair Identity" in Path(
        repair_dispatch.payload["briefing"]
    ).read_text(encoding="utf-8")

    role = orch._find_role_by_instance(original_child["role_instance"])  # type: ignore[attr-defined]
    assert role is not None
    replay_context = orch._dispatch_context(  # type: ignore[attr-defined]
        role=role,
        briefing_path=Path(repair_dispatch.payload["briefing"]),
        task_id="TASK-1",
        trace_id="trace-1",
        dispatch_id=repair_dispatch.payload["dispatch_id"],
    )
    replay = prepare_task_ref_repair_operation(
        orch,
        task=store.get("TASK-1"),  # type: ignore[arg-type]
        role=role,
        trigger_event=repair,
        dispatch_context=replay_context,
    )
    assert replay is not None
    assert replay.prepared.operation_id == new_request.payload["operation_id"]
    assert replay.prepared.request_hash == new_request.payload["request_hash"]
    assert replay.prepared.ensure_status == "running"
    assert len([
        event
        for event in log.read_all()
        if event.type == "workflow.operation.requested"
        and event.payload.get("operation_id") == replay.prepared.operation_id
    ]) == 1

    repaired_child = _child(_manifest(state_dir, fanout_id), "TASK-1")
    source_manifest = hydrate_sidecar_ref(
        state_dir,
        repaired_child["attempt_source_manifest"],
    ).payload
    for required_read in repaired_child["required_reads"]:
        read_attempt_artifact(
            state_dir,
            manifest=source_manifest,
            source_id=required_read["source_id"],
    ...
    completion = ZfEvent(
        type="dev.build.done",
        actor=repaired_child["role_instance"],
        task_id="TASK-1",
        correlation_id="trace-1",
        payload={
            "dispatch_id": repair_dispatch.payload["dispatch_id"],
            ...
            "child_id": original_child["child_id"],
            "run_id": old_run_id,
            "role_instance": repaired_child["role_instance"],
            "source_commit": commit,
            "source_branch": repaired_child["source_branch"],
            "workdir": repaired_child["workdir"],
            "files_touched": ["a.txt"],
            "summary": "repaired completion carrying stale worker call identity",
            "operation_id": old_operation_id,
            "request_hash": old_request_hash,
            "attempt_id": repair_dispatch.payload["attempt_id"],
            "result_protocol_mode": "blocking",
            **{
                key: repaired_child[key]
                for key in (
                    "workflow_run_id",
                    "contract_revision",
                    "task_map_generation",
    ...
    assert any(
        ...
        and event.task_id == "TASK-1"
        and event.payload.get("operation_id") == new_request.payload["operation_id"]
        for event in events
    )
    assert any(
    ...
    assert any(
        event.type == "fanout.child.completed"
        and event.payload.get("fanout_id") == fanout_id
        and event.payload.get("child_id") == original_child["child_id"]
        for event in events
    )
...
def test_affinity_slot_waits_for_previous_provider_turn_stop(tmp_path: Path):
    state_dir, log, transport, orch = _state(
        tmp_path,
        affinity_stage_slots=True,
    )
    ...
    orch._active_provider_turn = (  # type: ignore[method-assign]
        lambda instance_id: (
            {"turn_id": "turn-old"}
            if provider_turn_active and instance_id == task2["role_instance"]
            else None
        )
    )
    commit2 = _commit(Path(task2["workdir"]), "b.txt", "TASK-2\n", "TASK-2")
    ...
    waiting_manifest = _manifest(state_dir, fanout_id)
    assert _child(waiting_manifest, "TASK-3")["status"] == "queued"
    assert [sent[0] for sent in transport.sent] == ["dev-1", "dev-2"]
    assert [
    ...
    final_manifest = _manifest(state_dir, fanout_id)
    assert _child(final_manifest, "TASK-3")["status"] == "dispatched"
    assert [sent[0] for sent in transport.sent] == [
        "dev-1",
        "dev-2",
        "dev-2",
...
def test_fanout_dispatch_defers_while_provider_turn_is_active(tmp_path: Path):
    ...
    orch._active_provider_turn = (  # type: ignore[method-assign]
        lambda instance_id: (
            {"turn_id": "turn-old"} if instance_id == "dev-1" else None
        )
    )

    allowed = orch._ensure_fanout_role_dispatchable(
        role=role,
...
def test_provider_native_turn_close_releases_fanout_dispatch(tmp_path: Path):
    ...
    log.append(ZfEvent(
        ...
        actor="dev-1",
        payload={"session_id": "session-1", "turn_id": "turn-1"},
    ))
    ...
    log.append(ZfEvent(
        type="provider.turn.closed",
        actor="dev-1",
        payload={"backend": "codex", "turn_id": "turn-1"},
    ))

    assert orch._ensure_fanout_role_dispatchable(
        role=role,
        fanout_id="fanout-next",
        stage_id="dev-fanout",
        child_id="child-next",
...
def test_worker_recycle_releases_only_its_provider_turn_dispatch(
    tmp_path: Path,
):
    _state_dir, log, _transport, orch = _state(tmp_path)
    role = next(
    ...
    log.append(ZfEvent(
        ...
        actor="dev-2",
        payload={"instance_id": "dev-2", "backend": "codex"},
    ))
    assert not orch._ensure_fanout_role_dispatchable(
        role=role,
        fanout_id="fanout-next",
    ...
    log.append(ZfEvent(
        type="worker.recycled",
        actor="dev-1",
        payload={"instance_id": "dev-1", "backend": "codex"},
    ))
    assert orch._ensure_fanout_role_dispatchable(
        role=role,
        fanout_id="fanout-next",
...
def test_worker_recycle_preserves_new_provider_turn_started_during_recycle(
    tmp_path: Path,
):
    _state_dir, log, _transport, orch = _state(tmp_path)
    role = next(
...
def test_writer_failure_seals_committed_partial_head_for_semantic_replan(
    tmp_path: Path,
) -> None:
    state_dir, log, _transport, orch = _state(tmp_path)
    _seed_tasks(state_dir)
    _start(orch)
    fanout_id = _fanout_id(log)
    task = _child(_manifest(state_dir, fanout_id), "TASK-2")
    workdir = Path(task["workdir"])
    partial_head = _commit(
        workdir,
        "b.txt",
        "committed partial delivery\n",
        "partial delivery",
    )
    (workdir / "local-diagnostic.txt").write_text(
        "not committed\n",
        encoding="utf-8",
    )
    blocked = ZfEvent(
        ...
        payload={
            ...
            "run_id": task["run_id"],
            "pdd_id": "F-11111111",
            "reason": "contract requires one additional path",
        },
    )
    ...
    assert failed.payload["partial_head_commit"] == partial_head
    assert failed.payload["continuation_commit"] == partial_head
    assert failed.payload["partial_source_branch"] == task["source_branch"]
    assert failed.payload["continuation_ref"] == task["source_branch"]
    assert failed.payload["partial_worktree_clean"] is False
    assert failed.payload["partial_dirty_files"] == ["local-diagnostic.txt"]
    assert failed.payload.get("base_commit") != partial_head
...
def test_writer_candidate_ready_payload_matches_schema_contract(tmp_path: Path):
    ...
    fanout_id = _fanout_id(log)
    manifest = _manifest(state_dir, fanout_id)
    assert manifest["trigger_payload"]["request_id"] == "req-prd"
    assert manifest["trigger_payload"]["flow_kind"] == "prd"
    commits = {}
    for task_id, file_name in (("TASK-1", "a.txt"), ("TASK-2", "b.txt")):
        task = _child(manifest, task_id)
        commits[task_id] = _commit(
