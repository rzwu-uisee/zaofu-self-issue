...
def test_synthesized_agent_usage_ignores_cancelled_fanout_child(
    tmp_path: Path,
):
    from zf.core.config.schema import RoleConfig
    from zf.core.events.log import EventLog
    ...
    log.append(ZfEvent(
        ...
        actor="zf-cli",
        payload={"fanout_id": "fanout-old", "reason": "superseded"},
    ))
    host = _Host(log, task_store)
    role = RoleConfig(name="dev", backend="codex", instance_id="dev-lane-1")
    usage = SimpleNamespace(
    ...
    usage_evt = [event for event in log.read_all() if event.type == "agent.usage"][0]
    assert usage_evt.task_id is None
    assert usage_evt.payload.get("task_id") == ""
