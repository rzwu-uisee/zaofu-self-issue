...
Last manually verified: `2026-08-26`.
...
| `web-terminal-pty`<br>Web Terminal real Coding Agent PTY | `partial` | [`24-web-terminal.md`](../24-web-terminal.md)<br>[`24-web-terminal.en.md`](../24-web-terminal.en.md) | `src/zf/web/terminal_routes.py`<br>`src/zf/web/terminal_service.py`<br>`src/zf/web/terminal_usage.py`<br>`web/src/components/terminal/TerminalDrawer.tsx`<br>`web/src/components/agent-view/terminalUsageRows.ts` | `tests/test_web_terminal_runtime.py`<br>`tests/test_web_terminal_routes.py`<br>`tests/test_web_terminal_usage.py`<br>`web/tests/web-terminal.spec.ts`<br>`web/tests/terminalUsageRows.test.ts`<br>`web/tests/web-terminal-real-usage.spec.ts` |
...
| `evidence-bound-self-evolution`<br>Evidence-bound self-evolution and capability accumulation | `partial` | [`23-self-evolution-learning.md`](../23-self-evolution-learning.md)<br>[`23-self-evolution-learning.en.md`](../23-self-evolution-learning.en.md) | `src/zf/runtime/evolution_automation.py`<br>`src/zf/runtime/evolution_automation_support.py`<br>`src/zf/runtime/evolution_coordinator.py`<br>`src/zf/runtime/evolution_learning.py`<br>`src/zf/runtime/evolution_skill.py`<br>`src/zf/runtime/evolution_skill_automation.py`<br>`src/zf/runtime/evolution_skill_campaign.py`<br>`src/zf/runtime/evolution_skill_feedback.py`<br>`src/zf/runtime/evolution_skill_measurement.py`<br>`src/zf/runtime/evolution_skill_overlay.py`<br>`src/zf/runtime/evolution_skill_optimizer.py`<br>`src/zf/runtime/evolution_skill_optimizer_agent.py`<br>`src/zf/runtime/evolution_skill_optimizer_automation.py`<br>`src/zf/runtime/evolution_skill_optimizer_contracts.py`<br>`src/zf/runtime/evolution_skill_provider.py`<br>`src/zf/runtime/evolution_skill_source.py`<br>`src/zf/runtime/evolution_skill_trajectory.py`<br>`src/zf/runtime/evolution_trial_runner.py` | `tests/test_evolution_runtime.py`<br>`tests/test_evolution_learning.py`<br>`tests/test_evolution_environment.py`<br>`tests/test_evolution_skill_overlay.py`<br>`tests/test_evolution_skill_optimizer.py`<br>`tests/test_evolution_skill_optimizer_closure.py`<br>`tests/test_evolution_skill_trajectory.py`<br>`tests/test_evolution_skill_trust.py`<br>`tests/e2e/test_skill_evolution_production_e2e.py`<br>`tests/e2e/test_self_evolution_unattended_e2e.py` |
...
### `controlled-workflow-synthesis` - Controlled Workflow synthesis and freeze
...
### `role-provider-session-lifecycle` - Role-scoped Provider session and on-demand lifecycle
...
- **Activation**: Install and qualify Herdr and the target Provider CLI. The Dashboard host capability is enabled by default and can be explicitly disabled; select a registered Project, then create a session from its derived provider menu.
...
- **Rollback**: While the capability remains enabled, explicitly stop only the affected terminal sessions; then disable the host capability and restart Web. Directly disabling the capability does not terminate an already-running CLI; preserve registry and usage receipts for diagnosis.
...
### `task-pipeline-v4` - Task Pipeline v4 and elastic Stage Workers
...
### `research-generation-five-workflow-terminal` - Research generation and five-Workflow terminal acceptance
...
### `provider-native-operations-observability` - Provider native telemetry, Runtime Logs, and Operations metrics
...
- **Readback**: Inspect Operations capability/exporter health, the bounded redacted Runtime Logs API, and token-gated low-cardinality `/metrics`; confirm Delivery facts separately.
...
- **Readback**: In Delivery Graph, inspect each Claim's covering canonical Tasks, Plan/Implementation/Verification/Closure axes, open Gaps, and generation freshness on the single lightweight coverage surface.
...
### `evidence-bound-self-evolution` - Evidence-bound self-evolution and capability accumulation

- **Activation**: Enable Autoresearch resident and the explicit runtime.evolution policy, then validate the configured sealed evaluator root and credential environment variable name.
- **Readback**: Inspect evolution status, EventLog lineage, attempt/trial/canary artifacts, capability state, and post-reuse usage outcomes.
- **Rollback**: Disable evolution or return it to evaluate_only, then revoke assets through receipt-bound controlled transitions while preserving all evidence.
- **Authority**: Evolution is an evidence and capability lifecycle. It cannot rewrite product Task/Delivery truth; scoped Skill overlays may be auto-revoked, while canonical Skill optimization, replacement, deactivation, or deletion requires an owner-approved source action and parity receipt.
